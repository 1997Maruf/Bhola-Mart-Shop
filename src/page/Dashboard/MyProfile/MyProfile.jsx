...
const MyProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      axiosPublic.put(`/users/?email=${user?.email}`, userInfo).then((res) => {
  ...
  return (
    <div className="card card-compact w-96 bg-base-100 shadow-2xl	 m-auto mt-16">
      <figure>
        <img
          className="rounded-full w-48 h-48 mt-8 p-5 shadow-inner		shadow-slate-600"
          src={userInfo?.image}
          alt="Shoes"
        />
      </figure>
      <div className="card-body ">
        <h2 className=" text-[14px] ">{userInfo?.name}</h2>
        <p>
          <span>Email : </span>
          {userInfo?.email}
        </p>
